...
void WebServerManager::updateConfig() {
  ...
  config_.saveConfig(); motor_.applyConfig(config_.get()); inputs_.applyConfig(config_.get()); measurement_.applyConfig(config_.get(), mechanics); if (wifi) network_.requestReconnect(config_.get());
  JsonDocument out; out["message"] = mechanics ? "Настройки сохранены; требуется повторная калибровка." : (wifi ? "Настройки Wi-Fi сохранены; выполняется переподключение, fallback AP остаётся доступен." : "Настройки сохранены."); String body; serializeJson(out, body); server_.send(200, "application/json", body);
}
void WebServerManager::resetConfig() { config_.resetConfigToDefaults(); config_.saveConfig(); motor_.applyConfig(config_.get()); inputs_.applyConfig(config_.get()); measurement_.applyConfig(config_.get(), true); network_.requestReconnect(config_.get()); server_.send(200, "application/json", "{\"message\":\"Настройки сброшены; требуется калибровка.\"}"); }
